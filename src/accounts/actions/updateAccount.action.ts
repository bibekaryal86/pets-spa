...
import { Account, AccountsRequest, AccountsResponse } from '../types/accounts.data.types';
import { numberFormatter } from '../utils/accounts.utils';

export const updateAccount = (username: string, id: string, account: Account, method: string) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>): Promise<void> => {
    ...
    try {
      ...
      const updateAccountResponse = (await prefetch(urlPath, options)) as AccountsResponse;
