...
export const validateTransaction = (oneTransactionOne: OneTransactionOne): OneTransactionValidationMessages => {
  ...
  if (isTransactionTypeTransfer(oneTransactionOne.transactionTypeId)) {
    ...
    if (
      ...
      txnTypeTransferMessages += '[Transaction Type Transfer can be added to Category Type Transfer only]';
    }
  } else {
    if (
      !oneTransactionOne.merchantId &&
      (!oneTransactionOne.newMerchant || oneTransactionOne.newMerchant.trim().length === 0)
    ) {
    ...
    if (
      ...
      otherMessages += '[Merchant Name Should Begin With Alpha-Numeric Character]';
    }
  }
